...
BONI_AVAIL = [1, 3, 4, 5, 6, 7, 9, 10, 12, 13, 15, 16, 17, 18, 19, 21, 22, 23, 27, 28, 29, 30, 31, 32, 33, 34, 36, 37,
              39, 41, 43, 44, 45, 48, 53, 71, 72]

# Hier 6/7 Boni angeben!
BONI_RARE_AVAIL = BONI_AVAIL + [63]
...
mainfolder = "switchbot_v/"
if not os.path.exists(mainfolder):
    os.makedirs(mainfolder)
    os.makedirs(mainfolder + "1")
    os.makedirs(mainfolder + "2")
    for x in xrange(7):
        os.makedirs(mainfolder + "2/" + str(x))
...
class BonusSelector(ui.Bar):
    def Activate(self):
        ...
        if self.parentWindow.parentWindow.gameWindow:
    ...
    def Deactivate(self):
        ...
        if self.sub_parent.parentWindow.parentWindow.gameWindow:
    ...
    def Activate_rare(self):
        ...
        self.sub_parent.StatusText_rare.SetText("Jolla Ativo")
        self.Starter_rare_boni.SetText("Jolla Parar")
    ...
    def Deactivate_rare(self):
        ...
        self.sub_parent.StatusText_rare.SetText("Jolla Inativo")
        self.Starter_rare_boni.SetText("Jolla Iniciar")
    ...
    def Finish(self):
        ...
        self.Starter.SetText("Switchen (" + str(self.index + 1) + ") Continuar")
        self.sub_parent.boni_active = 0
        if self.sub_parent.parentWindow.parentWindow.gameWindow:
    ...
    def Finish_rare(self):
        ...
        self.sub_parent.StatusText_rare.SetText("Jolla Pronto")
        self.Starter_rare_boni.SetText("Jolla Continuar")
        self.sub_parent.boni_rare_active = 0
        if self.sub_parent.parentWindow.parentWindow.gameWindow:
    ...
    def __init__(self, sub_parent):
        ...
        self.EnableRareBoni = CheckBox("Jolla")
        self.EnableRareBoni.SetParent(self)
        self.EnableRareBoni.SetPosition(10, 285)
        ...
        self.Starter.SetPosition(0, 140)
        self.Starter.SetText("Switchen Iniciar")
        ...
        for vas in [7, 6]:
            ...
            self.boni[vas][0].SetText("Bônus " + str(vas))
            self.boni[vas][0].SetPosition(15, 10 + 25 * (vas - 1))
            self.boni[vas][0].Hide()

            self.boni[vas][1] = DropDown(self, "- Jolla -")
        ...
        for vas in range(5, 0, -1):
            ...
            self.boni[vas][1] = DropDown(self, "- Nenhum -")
            self.boni[vas][1].SetPosition(70, 10 + 25 * (vas - 1))
            for x in AFFECT_DICT:
                if x in BONI_AVAIL:
                    self.boni[vas][1].AppendItem(str(AFFECT_DICT[x](0)), x)
        ...
        self.save_vorlage = ui.Button()
        self.save_vorlage.SetParent(self)
        self.save_vorlage.SetPosition(380, 310)
        self.save_vorlage.SetUpVisual("d:/ymir work/ui/public/small_button_01.sub")
        self.save_vorlage.SetOverVisual("d:/ymir work/ui/public/small_button_02.sub")
        self.save_vorlage.SetDownVisual("d:/ymir work/ui/public/small_button_03.sub")
        self.save_vorlage.SetText("Salvar")
        self.save_vorlage.SetToolTipText("Salvar modelo", 60, 2)
        self.save_vorlage.SetEvent(self.Save_Data, self.boni)
        self.save_vorlage.Show()

        self.delete_vorlage = ui.Button()
        self.delete_vorlage.SetParent(self)
        self.delete_vorlage.SetUpVisual("d:/ymir work/ui/public/close_button_01.sub")
        self.delete_vorlage.SetOverVisual("d:/ymir work/ui/public/close_button_02.sub")
        self.delete_vorlage.SetDownVisual("d:/ymir work/ui/public/close_button_03.sub")
        self.delete_vorlage.SetToolTipText("Excluir proposta", 0, -23)
        self.delete_vorlage.SetPosition(380 - self.delete_vorlage.GetWidth() - 2, 285 + 2)
        self.delete_vorlage.Hide()

        #############################################################
        self.but_propose = DropDown(self, 'Sugestões', 0)
        self.but_propose.SetParent(self)
        self.but_propose.SetPosition(380, 285)
        ...
        self.BlockText.SetText("Você não pode alternar este item.")
        self.BlockText.Show()
    ...
    def Save_Data(self, data):
        ...
        getshow = 0
        boniList = []
        AFFECT_DICT_REVERSED = {}
        for x in AFFECT_DICT:
            AFFECT_DICT_REVERSED[str(AFFECT_DICT[x](0))] = x
        for vas in range(5, 0, -1):
            if data[vas][1].GetText() != "- Bônus -" and data[vas][3].GetText() != "0":
                boniList.append(
                    [str(AFFECT_DICT_REVERSED[data[vas][1].GetText()]), data[vas][3].GetText()]
                )
                treffer += 1
        if not treffer and not getshow:
            chat.AppendChat(2, "Para salvar, insira os valores.")
            getshow += 1
        else:
            self.__LoadQuestionDialog(boniList)

    def __LoadQuestionDialog(self, liste):
        self.QuesttionDialog = ui.Board()
        self.QuesttionDialog.SetSize(280, 105)
        self.QuesttionDialog.SetCenterPosition()
        self.QuesttionDialog.Show()

        self.BlockText = ui.TextLine()
        self.BlockText.SetParent(self.QuesttionDialog)
        self.BlockText.SetWindowHorizontalAlignCenter()
        self.BlockText.SetHorizontalAlignCenter()
        self.BlockText.SetVerticalAlignCenter()
        self.BlockText.SetPosition(0, 25)
        self.BlockText.SetText("Digite um nome.")
        self.BlockText.Show()

        self.vorlageNameBar = ui.Bar("UI")
        self.vorlageNameBar.SetParent(self.QuesttionDialog)
        self.vorlageNameBar.SetSize(80, 20)
        self.vorlageNameBar.SetWindowHorizontalAlignCenter()
        self.vorlageNameBar.SetPosition(10, 25 + 20 - 5)
        self.vorlageNameBar.SetColor(0xC0000000)
        self.vorlageNameBar.Show()

        self.vorlageName = Edit2("- Exemplo -", 14)
        self.vorlageName.SetParent(self.vorlageNameBar)
        self.vorlageName.SetWindowHorizontalAlignCenter()
        self.vorlageName.SetHorizontalAlignCenter()
        self.vorlageName.SetVerticalAlignCenter()
        self.vorlageName.SetPosition(2, 8)
        self.vorlageName.SetSize(80, 20)
        self.vorlageName.Show()

        self.acceptbtn = ui.Button()
        self.acceptbtn.SetParent(self.QuesttionDialog)
        self.acceptbtn.SetPosition(-80, 68)
        self.acceptbtn.SetWindowHorizontalAlignCenter()
        self.acceptbtn.SetUpVisual("d:/ymir work/ui/public/middle_button_01.sub")
        self.acceptbtn.SetOverVisual("d:/ymir work/ui/public/middle_button_02.sub")
        self.acceptbtn.SetDownVisual("d:/ymir work/ui/public/middle_button_03.sub")
        self.acceptbtn.SetText("Ok")
        self.acceptbtn.SetEvent(self.Accept, liste)
        self.acceptbtn.Show()

        self.cancelbtn = ui.Button()
        self.cancelbtn.SetParent(self.QuesttionDialog)
        self.cancelbtn.SetPosition(+40, 68)
        self.cancelbtn.SetWindowHorizontalAlignCenter()
        self.cancelbtn.SetUpVisual("d:/ymir work/ui/public/middle_button_01.sub")
        self.cancelbtn.SetOverVisual("d:/ymir work/ui/public/middle_button_02.sub")
        self.cancelbtn.SetDownVisual("d:/ymir work/ui/public/middle_button_03.sub")
        self.cancelbtn.SetText("Abortar")
        self.cancelbtn.SetEvent(self.QuesttionDialog.Hide)
        self.cancelbtn.Show()

    def GetProposalPath(self):
        item.SelectItem(self.sub_parent.vnum)
        type_1 = item.GetItemType() == 1 and "switchbot_v/" + str(item.GetItemType()) + "/"
        type_2 = item.GetItemType() == 2 and "switchbot_v/" + str(item.GetItemType()) + "/" + str(
            item.GetItemSubType()) + "/"
        return type_1 or type_2

    def Accept(self, liste):
        boniliste = liste
        if self.vorlageName.GetText() == "0":
            chat.AppendChat(2, "O campo de entrada deve ser preenchido.")
        else:
            try:
                item.SelectItem(self.sub_parent.vnum)
                dataname = self.GetProposalPath() + self.vorlageName.GetText()
                with open(dataname + ".txt", "w") as file:
                    file.write(';'.join(str(index[0]) + ',' + str(index[1]) for index in boniliste) + ';')

                chat.AppendChat(2, "Proposta: " + self.vorlageName.GetText() + " criado com sucesso.")
                self.QuesttionDialog.Hide()
                self.prepare_propose()
            except:
                chat.AppendChat(2, "Ao criar o arquivo " + dataname + ".txt houve um problema")

    def delete_vorlage_event(self, text):
        try:
            chat.AppendChat(2, "Proposta: " + text + " foi removido com sucesso.")
            os.remove(self.GetProposalPath() + text + ".txt")
        except:
            pass
        self.prepare_propose()
        self.but_propose.DropList.SelectIndex(0)
        self.delete_vorlage.Hide()
    ...
    def change_boni(self):
        ...
        ######################################################### Noa
        self.delete_vorlage.Show()
        self.delete_vorlage.SetEvent(self.delete_vorlage_event,
                                     str(self.but_propose.DropList.GetSelectedItem().textBox.GetText()))
        if self.but_propose.DropList.GetSelectedItem().textBox.GetText() == "Ler":
            self.delete_vorlage.Hide()
    ...
    def prepare_propose(self):
        ...
        self.but_propose.AppendItem("Ler", [])
                ...
                for files in os.listdir(path + item_type + "/" + item_subtype):
                    ...
                    for ItemList in liste:
                        if ItemList:
                            ...
                            proposals[int(item_type)][int(item_subtype)][files.replace(".txt", "")].append(
                                [bonusID, bonusVALUE])
        else:
...
class ItemTabBar(ui.Window):
    ...
    class ItemTab(ui.Bar):
        ...
        height_selected = 108 + 8
        height_unselected = 108 + 5
        rebuyed = 0
        rebuyed_rare = 0
        ...
        def __init__(self, parent, tabnum, index=0, vnum=0):  # Init ItemTab
            ...
            if self.parentWindow.parentWindow.gameWindow:
                ...
                self.blockBar.SetPosition(((ip2 - int(ip2 / 5) * 5) * self.blockBar.size_res),
                                          int(ip2 / 5) * self.blockBar.size_res)
            ...
            self.StatusText.SetText("Inativo")
            ...
            self.StatusText_rare.SetText("Jolla Inativo")
            ...
            self.bonusSelector.SetPosition(10, 35 + 108 + 8)
        ...
        def Select(self):
            ...
            self.parentWindow.parentWindow.enable_rebuy.SetPosition(10, 300 + 10)
            self.parentWindow.parentWindow.enable_rebuy.Show()
            self.parentWindow.parentWindow.but_speed.SetParent(self.bonusSelector)
            self.parentWindow.parentWindow.but_speed.SetPosition(0, 305 + 10)
        ...
        def nachkauf(self):
            if self.parentWindow.parentWindow.buyed:
                return
            else:
        ...
        def nachkauf_rare(self):
            if self.parentWindow.parentWindow.buyed_rare:
                return
            else:
        ...
        def Switch_rare(self):
            ...
            if not lc:
                if not self.parentWindow.parentWindow.enable_rebuy.checked:
                    chat.AppendChat(2,
                                    "Slot %d: Jolla Switchen abortado -> Nenhum item alternativo e compra adicional não foram bem-sucedidos!" % (
                                                self.index + 1))
                    self.bonusSelector.Deactivate_rare()
                    return
                self.bonusSelector.Deactivate_rare()
                chat.AppendChat(2, "Slot %d: Jolla Switches cancelados -> Não há itens de troca!" % (self.index + 1))
                return
        ...
        def Switch(self):
            ...
            if not lc and not self.parentWindow.parentWindow.enable_rebuy.checked:
                self.bonusSelector.Deactivate()
                return
        ...
        def checkSwitch(self):
            ...
            self.StatusText.SetText("Ativo (%d%%)" % self.prob)

            if ok or self.prob >= 90:
                if self.prob >= 90:
                    chat.AppendChat(1, "Slot %d: pronto!" % (self.index + 1))
                    self.bonusSelector.Finish()
                    return

                self.values = [player.GetItemAttribute(self.index, i) for i in [0, 1, 2, 3, 4]]
                self.Switch()
        ...
        def checkSwitch_rare(self):
            ok = False
            for i in [0, 1]:
                if player.GetItemAttribute(self.index, i + 5) != self.values_rare[i]:
                    ok = True

            self.prob_rare = self.GetProb_rare()
            self.StatusText_rare.SetText("Ativar (%d%%)" % self.prob_rare)
            if ok or self.prob_rare >= 90:
                if self.prob_rare >= 90:
                    self.bonusSelector.Finish_rare()
                    chat.AppendChat(1, "Slot %d: Jolla Pronto!" % (self.index + 1))
                    return

                self.values_rare = [player.GetItemAttribute(self.index, i) for i in [5, 6]]
                self.Switch_rare()
            ...
            elif not ok and self.count_rare < 3:
                self.count_rare += 1

            elif not ok and self.count_rare >= 2:
                self.count_rare = 0
                self.Switch_rare()

        def UpdateItem(self):
            item_index = player.GetItemIndex(self.index)
            if not item_index and self.vnum != 0:
                self.resetSwitch()
                self.resetSwitch_rare()
                self.vnum = 0
                self.bonusSelector.Deactivate()
                ...
                return
            elif item_index != self.vnum:
                self.resetSwitch()
                self.resetSwitch_rare()
                self.vnum = item_index
                self.bonusSelector.Deactivate()
                self.bonusSelector.prepare_propose()
                ...
                if item.GetItemType() not in [1, 2]:
                    self.bonusSelector.Block()
                else:
        ...
        def GetProb_rare(self):
            values = [player.GetItemAttribute(self.index, i + 5) for i in range(2)]
            val2 = {}
            for i in range(2):
                try:
                    affectString = AFFECT_DICT[values[i][0]](values[i][1])
                    val2[values[i][0]] = values[i][1]
                    self.bonusSelector.ibi[i + 5].SetText(affectString)
                except:
                    pass
            prob = 0
            max = 0
        ...
        def GetProb(self):
            values = [player.GetItemAttribute(self.index, i) for i in range(5)]
            val2 = {}
    ...
    def __init__(self, parent):
        ...
        self.AddText.SetText("Adicionar")
    ...
    def AddTab_pre(self):
        if mouseModule.mouseController.isAttached():
            ...
            if item.GetItemType() != 1 and item.GetItemType() != 2:
                ...
                chat.AppendChat(2, "Você não pode rodar este item.")
                return

            for a in self.tabList:
                if self.tabList[a].index == attachedSlotPos:
                    mouseModule.mouseController.DeattachObject()
                    chat.AppendChat(2, "Este slot já foi inserido!")
                    return
    ...
    def AddTab(self, id, vnum=0):
        ...
        self.parentWindow.SetSize(520, 487 + 10)
...
class Bot(ui.BoardWithTitleBar):
    class PinGroup(ui.Bar):
        ...
        def __init__(self, parent, dir=1):
            ...
            self.maximise_but.SetText("Modo Janela")
    ...
    def ShowPinHint(self, type):
        ...
        if type == 1:  # Left
            ...
            self.PinHint.SetPosition(0, 36 + max(min(30, 30 - (wndMgr.GetScreenHeight() - 36 - (y + self.GetHeight()))),
                                                 3))
        self.PinHint.Show()
    ...
    def __init__(self):
        ...
        self.SetTitleName("Switchbot SoW2")
        self.AddFlag("float")
        self.AddFlag("movable")
        ...
        self.titleBar.HelpBut.SetToolTipText("Ajuda detalhada", 0, -23)
        self.titleBar.HelpBut.SetText("Ajuda")
        ...
        self.but_deactivate_all.SetText("Parar tudo")
        ...
        self.titleBar.but_deactivate_all.SetPosition(
            3 + 5 + self.titleBar.HelpBut.GetWidth() + 5 + self.but_deactivate_all.GetWidth(), 0)
        ...
        self.enable_rebuy = CheckBox("Comprar")
        ...
        self.help_add_item = HelpBar(0.8, 'Arraste um item aqui para adicioná-lo.')
        self.help_add_item.SetParent(self)
        self.help_add_item.SetPosition(60, 50)

        self.help_minimize = HelpBar(0.8, 'Arraste o bot para a borda para torná-lo mais pequeno', 1)
    ...
    def _Hide(self):
        ## ONLY FOR TESTING
        # for a in range(self.itemTabBar.tabCount):
    ...
    def OnUpdate(self):
        if self.AboutWindow:
            if self.AboutWindow.x_counter > 1:
                ...
                self.AboutWindow.text6.SetText("Tempo: %0.1f" % (self.AboutWindow.x_counter / 45.0))
        ...
        if self.help_on == 1:
            ...
            if self.itemTabBar.tabCount:
                ...
                self.help_duration.SetPosition(
                    190 - 5.5 + self.but_speed.GetSliderPos() * int(35.5 + self.but_speed.GetWidth() / 2), 180)
    ...
    def about(self):
        ...
        self.AboutWindow.text2.SetText("Este switchbot foi desenvolvido por Mijago.")
        ...
        self.AboutWindow.text3.SetText("Novas ideias são sempre bem-vindas!")
        ...
        self.AboutWindow.text6.SetText("Tempo: %d" % self.AboutWindow.x_counter)
