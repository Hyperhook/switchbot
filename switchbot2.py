...
BONI_AVAIL = [1,3,4,5,6,7,9,10,12,13,15,16,17,18,19,21,22,23,27,28,29,30,31,32,33,34,36,37,39,41,43,44,45,48,53,71,72]

# Hier 6/7 Boni angeben!
BONI_RARE_AVAIL = [1,3,4,5,6,7,9,10,12,13,15,16,17,18,19,21,22,23,27,28,29,30,31,32,33,34,36,37,39,41,43,44,45,48,53,71,72,63]
...
class BonusSelector(ui.Bar):
	...
	def GetProposalPath(self):
		...
		type_2 = item.GetItemType() == 2 and "switchbot_v/"+str(item.GetItemType())+"/"+str(item.GetItemSubType())+"/"
	...
	def change_boni(self):
		...
		self.delete_vorlage.SetEvent(self.delete_vorlage_event, str(self.but_propose.DropList.GetSelectedItem().textBox.GetText()))
...
class ItemTabBar(ui.Window):
	...
	class ItemTab(ui.Bar):
		...
		def Switch_rare(self):
			...
			if not lc:
				if not self.parentWindow.parentWindow.enable_rebuy.checked:
					chat.AppendChat(2,
									"Slot %d: Jolla Switchen abortado -> Nenhum item alternativo e compra adicional não foram bem-sucedidos!" % (self.index + 1))
					self.bonusSelector.Deactivate_rare()
					return
				self.bonusSelector.Deactivate_rare()
		...
		def GetProb_rare(self):
			values = [player.GetItemAttribute(self.index, i+6) for i in range(2)]
			val2 = {}
			for i in range(2):
				try:
